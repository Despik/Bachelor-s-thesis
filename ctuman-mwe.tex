...
The example of the SCHC configuration is shown in following listing.
...
In this chapter I will introduce datasets for evaluation and present comparison of different heuristics and local search techniques.

\section{Datasets}

All of the following layout configurations and stacker crane attributes are based on real-world automated warehouse design. Expedition and production input data are based on anonymized real-world data from provided from a client for a project involving the simulator.


\subsection{Dataset A}
\label{subsec:dataA}
Dataset A is used in performance evaluation. The data (A1, A2, A3 and A4) will be used to compare performance of expeditions with different sizes. These expeditions have similar number of trucks, requested item types and distribution of types.\\

Common layout
\begin{itemize}
    \item Number of aisles: 48
    \item ...
\end{itemize}

Common stacker crane attributes
\begin{itemize}
    \item Vertical speed: ...
    \item ...
\end{itemize}
...
\label{seubsec:B}
Dataset B is used for comparison of different heuristics. These data are used:

B1 ...

B2 ...

The input production and expedition files are the ones from data A2 and it is same for every data in dataset B.

\section{Performance}

In this section we simply show how long does it take to create the schedule from input files A1, A2, A3 and A4 from dataset A (See Subsection \ref{subsec:dataA}). We take a look at phase two, three and four since the first phase have insignificant overall effect on execution time. For phases three and four, we measure the number of evaluated moves in half a minute. The following table shows the results.

\begin{center}
\begin{tabular}{ |c| c c c c|} 
\hline
phase/data & A1 & A2 & A3 & A4\\ 
\hline
phase 2, no ls evaluated & 371ms & 1673ms & 4623ms & 16820ms \\
\hline
phase 3 moves evaluated & 417 steps & 39 steps & 12 steps & 0 steps \\ 
\hline
phase 4 moves evaluated & 330 steps & 39 steps & 10 steps & 0 steps \\ 
\hline
\end{tabular}
\end{center}
\section{Heuristics comparison}

In this section schedules created from dataset B using different rules and heuristics are presented. For each schedule, we take a look at their utilization of machines by comparing makespan of these schedules (See Figure \ref{fig:c1}), before idle time insertion. To be able to tell how well the machines were utilized. We also show  the approximation of optimal makespan in each figure.  It assumes that throughput of the warehouse is limited only by a scanner and calculates how long does a scanner need to scan all items on the expedition expedition ramp that expects the most items. The calculation of this approximation can be seen in Equation \ref{eq:approx}.

Then we take a look at each schedule individually. Each schedule created by the scheduler is compared to the version of it that was constructed from activities at the end of the simulation.
...
\caption{Makespans of the LPT-LJL schedule created by the second phase of the proposed algorithm}
\label{fig:c1}
\end{figure}

\subsection{LPT-LJL}

This is the proposed heuristic from Chapter \ref{ch:Proposed solution}.

\subsubsection{Data B1}
\begin{center}
\begin{tabular}{ |c|| c | c |} 
\hline
attribute/schedule & B1 - LPT-LJL & B1 - result\\ 
\hline
score(hard/soft) & 0/0 & d\\
\hline
U & 0/0 & d\\
\hline
L & 0/0 & d\\
...
$\sigma$ & 0/0 & d\\
\hline
scanners max. buffer size reached & 0 & 10\\ 
\hline
average length of idle times & 0 & 0\\ 
\hline
\end{tabular}
\end{center}

\subsubsection{Data B2}
\begin{center}
\begin{tabular}{ |c|| c | c |} 
\hline
attribute/schedule & B2 - LPT-LJL & B2 - result\\ 
\hline
score(hard/soft) & 0/0 & d\\
\hline
U & 0/0 & d\\
...
\caption{Makespans of the SPT-LJL schedule created by the second phase of the proposed algorithm}
\label{fig:c2}
\end{figure}

\subsection{LPT + cyclic ramp and machine selection}

\begin{figure}[H]
\includegraphics[width=1\linewidth]{c3.png}
\caption{Makespans of the SPT schedule with cyclic machne and ramp selection created by the second phase of the proposed algorithm}
\label{fig:c3}
\end{figure}

\subsection{Random ordering and selection}

\begin{figure}[H]
\includegraphics[width=1\linewidth]{c4.png}
\caption{Makespans of the schedule with random job dispatching and ramp/machine selection created by the second phase of the proposed algorithm}
\label{fig:c4}
\end{figure}

\subsection{Original activity execution with no scheduler}

\begin{figure}[H]
\includegraphics[width=1\linewidth]{c4.png}
\caption{Makespan of the schedule created at the end of the simulation from activities without usage of the scheduler}
\label{fig:c4}
\end{figure}
...
table - buffers, idle times, etc. for all 4 of these


\subsection{Effect of objectives}
gantt charts before and after insertions

score after and before idle time insertions

    time and reached score + taking the worst score and the best score and compare it in simulation



Reality worst score vs reality best score


\subsection{Experimentation}
Taking information from simulation -> production uniformly distributed -> stochastic model using average processing time? What is the result?
Porovnani kdyz treba zvolim average processing time
\chapter{Conclusion}
